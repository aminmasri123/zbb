            ...
            <section class="rounded-xl border border-gray-200 bg-white shadow-sm">
                <div class="flex flex-col gap-3 border-b border-gray-200 p-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex gap-2">
                        ...
                        <Link v-if="can('materialanforderung.create')" :href="route('materialanforderung.create')" class="inline-flex shrink-0 items-center gap-2 rounded-lg bg-orange-600 px-4 py-2 text-sm font-semibold text-white"><i class="las la-plus"></i><span class="hidden sm:inline">Neue Anforderung</span></Link>
                    </div>
                </div>
                ...
                <div class="hidden overflow-x-auto md:block">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100"><tr v-for="item in filtered" :key="item.id" class="hover:bg-gray-50"><td class="px-4 py-3 font-semibold">#{{ item.id }}</td><td class="px-4 py-3">{{ date(item.created_at) }}</td><td class="px-4 py-3">{{ item.besteller?.name || '–' }}</td><td class="px-4 py-3"><span class="rounded-full px-2.5 py-1 text-xs font-semibold" :class="statusMeta[item.status]?.[1]">{{ statusMeta[item.status]?.[0] || item.status }}</span></td><td class="px-4 py-3"><span v-if="item.prioritaet === 'dringend'" class="mr-1 text-red-600" title="Dringend">●</span>{{ date(item.benoetigt_am) }}</td><td class="px-4 py-3">{{ item.kostenstelle }}</td><td class="px-4 py-3 text-right font-semibold">{{ euro(item.endsumme) }}</td><td class="px-4 py-3 text-right"><Link v-if="can('materialanforderung.show')" :href="route('materialanforderung.show', item.id)" class="rounded-lg border border-gray-300 px-3 py-1.5 font-semibold">Öffnen</Link></td></tr><tr v-if="filtered.length === 0"><td colspan="8" class="px-4 py-10 text-center text-gray-500">Keine Materialanforderungen gefunden.</td></tr></tbody>
                    </table>
                </div>

                <div class="divide-y divide-gray-100 md:hidden">
                    <article v-for="item in filtered" :key="item.id" class="p-4">
                        <div class="flex items-start justify-between gap-3"><div><p class="font-bold text-gray-900">Materialanforderung #{{ item.id }}</p><p class="text-sm text-gray-500">{{ item.besteller?.name || '–' }} · {{ date(item.created_at) }}</p></div><span class="shrink-0 rounded-full px-2 py-1 text-xs font-semibold" :class="statusMeta[item.status]?.[1]">{{ statusMeta[item.status]?.[0] || item.status }}</span></div>
                        <div class="mt-3 grid grid-cols-2 gap-3 text-sm"><div><span class="block text-xs text-gray-500">Kostenstelle</span>{{ item.kostenstelle }}</div><div><span class="block text-xs text-gray-500">Benötigt am</span>{{ date(item.benoetigt_am) }}</div><div><span class="block text-xs text-gray-500">Endsumme</span><strong>{{ euro(item.endsumme) }}</strong></div><div class="self-end text-right"><Link v-if="can('materialanforderung.show')" :href="route('materialanforderung.show', item.id)" class="inline-block rounded-lg bg-gray-900 px-3 py-2 font-semibold text-white">Öffnen</Link></div></div>
                    </article>
