...
readonly NEEDLE='    DEVELOPMENT = "get_participant_development_data"'
readonly INSERT='    POTENTIAL_ANALYSIS_SUPPORT_NEEDS = "get_participant_potential_analysis_support_needs"'
...
if grep -Fqx "${INSERT}" "${SOURCE_FILE}" && grep -Fqx "${INSERT}" "${runtime_file}"; then
    echo 'Schema unterstützt get_participant_potential_analysis_support_needs bereits.'
    exit 0
fi
...
backup="${BACKUP_ROOT}/$(date -u +%Y%m%dT%H%M%SZ)-pre-pa-support-tool-schema"
...
for _ in {1..30}; do
    if curl -fsS --max-time 2 http://127.0.0.1:8000/health/live >/dev/null; then
        trap - ERR
        echo 'Schema für get_participant_potential_analysis_support_needs wurde aktiviert.'
