...
ZBB_OLLAMA_REPORT_MODEL=qwen3:1.7b
...
ZBB_OLLAMA_TIMEOUT_SECONDS=600
EOF
    unset secret
fi
...
if grep -q '^ZBB_OLLAMA_REPORT_MODEL=' "${ENV_FILE}"; then
    sed -i 's/^ZBB_OLLAMA_REPORT_MODEL=.*/ZBB_OLLAMA_REPORT_MODEL=qwen3:1.7b/' "${ENV_FILE}"
else
    printf '%s\n' 'ZBB_OLLAMA_REPORT_MODEL=qwen3:1.7b' >> "${ENV_FILE}"
fi
if grep -q '^ZBB_OLLAMA_WORKSPACE_MODEL=' "${ENV_FILE}"; then
    sed -i 's/^ZBB_OLLAMA_WORKSPACE_MODEL=.*/ZBB_OLLAMA_WORKSPACE_MODEL=qwen3:1.7b/' "${ENV_FILE}"
else
    printf '%s\n' 'ZBB_OLLAMA_WORKSPACE_MODEL=qwen3:1.7b' >> "${ENV_FILE}"
fi
