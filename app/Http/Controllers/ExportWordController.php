...
class ExportWordController extends Controller
{
    ...
    public function teilnehmerDokumentPaket(
        Request $request,
        Personen $personen,
        DokumentPaket $paket,
        ActiveProjectContext $activeProjectContext
        ...
        abort_unless($paket->aktiv !== false, 404);
        abort_unless($paket->projekte()->whereKey($projekt->id)->exists(), 404);

        $paket->loadMissing('dokumente');
        if ($paket->dokumente->isEmpty()) {
            return back()->with('error', 'Das Dokumentenpaket enthält keine Vorlagen.');
        }
        ...
        foreach ($paket->dokumente as $dokument) {
            if (
                $dokument->aktiv === false
                || $dokument->kontext !== 'teilnehmer'
                || $dokument->einsatzbereich !== 'teilnehmer'
                || ! $this->isAssignedToProject($projekt, $dokument)
                || ! $this->canExportDocument($user, $dokument)
                || ! $this->formatAllowed($dokument, 'pdf')
            ) {
                $errors[] = $dokument->name.': Vorlage ist für dieses Projekt oder diesen Benutzer nicht als PDF freigegeben.';

                continue;
            }

            $templateFile = $dokument->dateipfad ? $this->storageTemplatePath($dokument->dateipfad) : null;
            if (! $templateFile || ! is_file($templateFile)) {
                $errors[] = $dokument->name.': Vorlagendatei wurde nicht gefunden.';

                continue;
            }

            if ($error = $this->participantTemplateValidationError($templateFile, $gruppe, $projekt, $personen, $dokument, $teilnahme)) {
                $errors[] = $dokument->name.': '.$error;

                continue;
            }

            $prepared[] = [$dokument, $templateFile];
        }

        if ($errors !== []) {
            return back()->with('error', "Das Paket kann nicht exportiert werden:\n- ".implode("\n- ", $errors));
        }

        $generatedFiles = [];
        try {
            foreach ($prepared as [$dokument, $templateFile]) {
                $path = $dokument->typ === 'pdf'
                    ? $templateFile
                    : $this->createParticipantTemplateOutput(
                        $templateFile,
                        $gruppe,
                        $projekt,
                        $personen,
                ...
                $generatedFiles[] = [
                    'path' => $path,
                    'temporary' => $dokument->typ !== 'pdf',
                    'name' => $this->safeFileName($dokument->name).'.pdf',
                ];
            }
            ...
            $baseName = $this->safeFileName($paket->name.'_'.$personen->vorname.'_'.$personen->nachname);

            if ($format === 'zip') {
                $outputPath = $tempDir.DIRECTORY_SEPARATOR.uniqid('teilnehmer_paket_', true).'.zip';
                $zip = new ZipArchive;
                ...
                foreach ($generatedFiles as $index => $file) {
                    ...
                    if (isset($usedNames[$name])) {
                        $name = pathinfo($name, PATHINFO_FILENAME).'_'.($index + 1).'.pdf';
                    }
                    $usedNames[$name] = true;
                ...
                $outputPath = $tempDir.DIRECTORY_SEPARATOR.uniqid('teilnehmer_paket_', true).'.pdf';
                app(PdfMerger::class)->merge(array_column($generatedFiles, 'path'), $outputPath);
            ...
            return back()->with('error', 'Dokumentenpaket konnte nicht erstellt werden: '.$exception->getMessage());
        }
    ...
    private function downloadParticipantTemplate(
        ...
        try {
            $outputPath = $this->createParticipantTemplateOutput(
                $templateFile,
                $gruppe,
                $projekt,
                $personen,
    ...
    private function participantTemplateValidationError(
        string $templateFile,
        Gruppe $gruppe,
        Projekt $projekt,
        Personen $personen,
        ...
        if ($dokument->typ === 'word') {
            ...
            $variables = $this->spreadsheetPlaceholderVariables(SpreadsheetIOFactory::load($templateFile));
        } else {
            return 'Dieser Vorlagentyp wird für Teilnehmerexporte nicht unterstützt.';
        }

        return $this->singleParticipantPlaceholderValidationError($variables, $values, $personen);
    }

    private function createParticipantTemplateOutput(
        string $templateFile,
        Gruppe $gruppe,
        Projekt $projekt,
        Personen $personen,
        ...
        try {
            if ($validate && ($error = $this->participantTemplateValidationError(
                $templateFile,
                $gruppe,
                $projekt,
                $personen,
            ...
            if ($dokument->typ === 'word') {
                ...
                throw new \RuntimeException('Dieser Vorlagentyp wird für Teilnehmerexporte nicht unterstützt.');
            }
        } catch (Throwable $exception) {
        ...
        if (! $outputPath || ! is_file($outputPath)) {
            throw new \RuntimeException('Die Ausgabedatei wurde nicht erstellt.');
        }
    ...
    /** @return array{missing:Collection, unknown:Collection} */
